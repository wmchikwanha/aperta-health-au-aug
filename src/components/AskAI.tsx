...
export function AskAI({ context, contextLabel }: AskAIProps) {
  ...
  // THIS IS THE RESTORED AND UPDATED FUNCTION
    ...
    try {
      ...
      if (!accessToken) {
        toast({
          ...
          description: "Please sign in to use Ask AI.",
          variant: "destructive"
        });
        return;
      }
      ...
      const response = await fetch(CHAT_URL, {
        ...
        // We use 'message' to match the Edge Function's expected key
      ...
      // Extract content from Gemini/OpenAI format (choices[0].message.content)
    ...
    <Card className="fixed bottom-4 right-4 w-96 h-[600px] shadow-2xl flex flex-col">
      <div className="flex items-center justify-between p-4 border-b">
        ...
        <div className="flex gap-1">
          ...
          <Button variant="ghost" size="icon" onClick={handleClose} title="Close and clear chat">
            <X className="h-4 w-4" />
          </Button>
        </div>
      </div>
